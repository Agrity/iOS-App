...
class CoreDataTableViewController: UITableViewController, NSFetchedResultsControllerDelegate
{
    ...
    override func tableView(tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        if let sections = fetchedResultsController?.sections where sections.count > 0 {
            return getHeaderTitle(sections[section].name)
        } else {
            return nil
        }
    }
    
    // MARK - update if I decide I want indexing
...
//    }
    ...
    private func getHeaderTitle(type: String) -> String {
        switch type {
            case "1": return "Accepted Bids"
            case "2": return "Rejected Bids"
            case "3": return "Expired Bids"
        default: return ""
        }
    }
}